...
## Unreleased
- **Launcher dock**: `dock` widget (app icons from desktop entries, click to launch; hover zoom/tooltips or labels), `desktop-widgets apps`, editor app picker; registry types may declare `input: true` and per-type `defaults` for common keys; `top-center` / `bottom-center` corners.
